...
#                        test_labels=array[per[train_size:],-1]  
                ...
                ts=seq[0]
#                seq_m[node][benchmark][m]=ts

                M=np.arange(1,9)
                seq=np.asarray(seq)
                
#                plt.scatter(np.arange(2,9),[seq[0]/seq[i] for i in range(1,len(seq))])
                
#                gd=[1024,2048,3792,4096,4816,5536,6560,7584,8304,6932,9328,10352,11072,13120,13840,15168,16608,18656]
                results=[[],[],[],[],[]]
                for gd in g_params[node][benchmark].keys():
                    M=np.arange(1,9)

                    if len(g_params[node][benchmark][gd].keys())==8:                        
                        seq=np.asarray([g_params[node][benchmark][gd][th][0] for th in M])
#                        for i in range(1,9):
#                            if g_params[node][benchmark][gd][i][1]<i:
#                                M[i-1]=g_params[node][benchmark][gd][i][1]
                                
                        model=uslfit(M,seq)
                        results[0].append(gd)
                        results[1].append(model[0])
                        results[2].append(model[1][0])
                        results[3].append(model[1][1])
                        results[4].append(M.tolist())
                        
                tm=g_params[node][benchmark][mflop][1][0] 
                def my_func_3d_6(data,alpha):   
                    tm=g_params[node][benchmark][mflop][1][0]
                    kappa=0.
                    N=data[:,1]
                    n_t=data[:,0]
                    ...
                    M=np.minimum(n_t,N) 
                    L=np.ceil(n_t/(N))
                    return (tm-alpha)*(w_c/mflop)+alpha*L
#                popt6, pcov=curve_fit(my_func_3d_6,train_set,train_labels,method='lm')
#                j=1
#                for gd in g_params[node][benchmark].keys():
#                    plt.figure(j)
#                    for i in g_params[node][benchmark][gd]:
#                        plt.scatter(i,mflop/g_params[node][benchmark][gd][i][0])
#                        plt.xlabel('threads')
#                        plt.ylabel('Mflops')
#                        plt.title('grain size:'+str(int(gd)))
#                    j=j+1
#                plt.scatter(results[0],results[1],label='ts')
#                plt.grid(True,'both')
#                plt.xscale('log')
#                plt.xlabel('Grain size')
#                plt.ylabel('ts')
#                
#                plt.scatter(results[0],results[2],label='m[0]')
#                plt.grid(True,'both')
#
#                plt.xscale('log')
#                plt.xlabel('Grain size')
#                plt.ylabel('Contention')
##                plt.scatter(results[0],[np.exp((x-mflop)/mflop) for x in results[0]],label='exp-m[0]')
##                plt.xscale('log')
#
#                plt.scatter(results[0],results[3],label='m[0]')
#                plt.grid(True,'both')
#                plt.xlabel('Grain size')
...
#
#                gd=1024.
#                seq2=np.asarray([g_params[node][benchmark][gd][th] for th in range(1,9)])
#                model2=uslfit(M,seq2)
...
#                    m,_,_,_= np.linalg.lstsq(Q, (t/ts)-1/n)
                    return (ts,m)            
                
                def uslvalue(x, m): 
                    ts=m[0]
                    model=m[1]
                    y=ts*(1+model[0]*(x-1)+model[1]*x*(x-1))/x
                    return y
                
#                model=uslfit(M,seq)
#                uslvalue(M,model)
#                plt.scatter(matrix_sizes, seq_m,label='dmatdmatadd')
#                plt.plot(matrix_sizes,y(matrix_sizes))
##                plt.plot(matrix_sizes, seq_m_3,label='dmatdmatdmatadd')
##                plt.plot(matrix_sizes, [3/2*s for s in seq_m],label='3/2dmatdmatadd')
#
...
#                model=m2fit(np.asarray(matrix_sizes),np.asarray(seq_m))
#                y=m2val(np.asarray(matrix_sizes),model)
...
#                l2=32*1024
#                l3=20*(2**10)/8
#                def matrix_func(x,a,b):
#                    point=853.33
#                    return a*x*x+b#+c*(x-point)*np.heaviside(x-point,1)#+d*(x-point)*np.heaviside(x-point,1)
#                popt_m, pcov=curve_fit(matrix_func,np.asarray(matrix_sizes),np.asarray(seq_m[node][benchmark][m]),method='lm')
#                
#                plt.scatter(matrix_sizes,matrix_func(matrix_sizes,*popt_m))
#                plt.scatter(matrix_sizes, [seq_m[node][benchmark][m] for m in matrix_sizes],label='dmatdmatadd')
...
#                    return alpha*L+(ts+ts*gamma*(M-1)+ts*kappa*M*(M-1))*(w_c)/mflop+h*n_t*np.heaviside(n_t-N,1)+d*(1-ws)/N#+q*(n_t-1)*(g-mflop%g)/g*np.heaviside(N-n_t,1)#+d1*(ts)*(g-(mflop%g))*(1/mflop)

                
    
                i=1
                #alpha,gamma,d,h,q,ts
...
#                popt3, pcov=curve_fit(my_func_3d_3_no_ws,train_set,train_labels,method='lm')
                popts.append(popt3)
#                popt4, pcov=curve_fit(my_func_3d_4,train_set,train_labels,method='lm')

                ts=g_params[node][benchmark][mflop][1][0]
#                popt5, pcov=curve_fit(my_func_3d_5,train_set,train_labels/ts,method='lm')
...
#                    z3=my_func_3d_3_no_ws(new_array,*popt3)
#
##                    z4=my_func_3d_4(new_array,*popt4)
##                    z5=my_func_3d_5(new_array,*popt5)
#
#                    plt.figure(i)
#                    plt.axes([0, 0, 2, 1])
...
#                    plt.scatter(new_array[:,0],z3,label='pred3',marker='.',color='red')
###                    plt.scatter(new_array[:,0],z4,label='pred4',marker='.')
##                    plt.scatter(new_array[:,0],ts*z5,label='pred5',marker='.',color='green')
#
#                    plt.xscale('log')
...
#                    plt.scatter(new_array[:,-1],z3,label='pred2',marker='.',color='red')
...
#                
#                def softplus(x,a,b):
#                    return np.log(1+np.exp(-np.abs(a*(x-b))))+np.maximum(a*(x-b),0)
##                    return np.log(1+np.exp(a*(x-b)))
#                ms=np.asarray(matrix_sizes)
#                plt.plot(ms,softplus(ms,p[0],948))
#                
##                
#                def uslfit(n, t):           
#                    Q = np.zeros((t.size, 2))
...
[alpha,gamma,q,d,h,ts]=popt_i
ts=my_func_t_seq(m,*popt_seq)
z6=mflop/my_func_3d_3(new_array,alpha,gamma,q,d,h,ts)


plt.plot(matrix_sizes[:12],[ts*(m**2)/(690*692) for m in matrix_sizes[:12]])
popts_k=np.load(perf_directory+'/'+str(version)+'/'+node+'/'+runtime+'_'+node+'_'+benchmark+'_popts_'+str(version)+'.npy') 
labels=['alpha','gamma','d','h','q','ts']
for j in range(len(labels)):
    plt.figure(j)
    plt.scatter(matrix_sizes[:],[popts[i][j] for i in range(len(matrix_sizes[:]))],label=labels[j])
#    plt.plot(matrix_sizes[:12],[ts*(m**2)/(690*692) for m in matrix_sizes[:12]])

    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)    

plt.scatter(matrix_sizes[:],[popts[i][5]/popts[i][4] for i in range(len(matrix_sizes[:]))],label=labels[j])

def my_func_t_seq(x,a,b,c):
    return a*x**2+b*x+c
popt_seq, pcov=curve_fit(my_func_t_seq,matrix_sizes[:12],[popts[i][j] for i in range(len(matrix_sizes[:12]))],method='lm')
plt.scatter(matrix_sizes[:12],[popts[i][j] for i in range(len(matrix_sizes[:12]))],label=labels[j])
plt.scatter(matrix_sizes[:12],my_func_t_seq(matrix_sizes[:12],*popt_seq),label=labels[j])

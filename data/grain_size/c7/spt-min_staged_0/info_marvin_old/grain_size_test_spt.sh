...
#thr=(8)
#num_iterations=()
num_iterations=(100000 1000000 10000000 100000000)
iter_lengths=(1)
...
do
	...
	echo " task length ${il}"
		for th in ${thr[@]}
			...
			if [ $counter == 1 ]
			then
				${hpx_bin_dir}/grain_size_test --spt --counter -Ihpx.stacks.use_guard_pages=0 --num_iterations=${ni}  --hpx:threads=${th} --iter_length=${il} --repetitions=0 --hpx:print-counter=/threads/idle-rate  --hpx:print-counter=/threads/time/average --hpx:print-counter=/threads/time/cumulative-overhead --hpx:print-counter=/threads/count/cumulative --hpx:print-counter=/threads/time/average-overhead>>${results_dir}/${node}-sptctr_grain_size_${th}_${c}_${il}_${ni}.dat
			else
				${hpx_bin_dir}/grain_size_test --spt  -Ihpx.stacks.use_guard_pages=0 --num_iterations=${ni}  --hpx:threads=${th} --iter_length=${il} --repetitions=6 --hpx:ini=hpx.thread_queue.min_tasks_to_steal_staged=0>>${results_dir}/${node}-spt_grain_size_${th}_${c}_${il}_${ni}.dat

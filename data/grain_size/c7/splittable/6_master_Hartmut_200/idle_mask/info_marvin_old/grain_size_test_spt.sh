...
do
	...
	echo " task length ${il}"
		for th in ${thr[@]}
			...
			if [ $counter == 1 ]
			then
				${hpx_bin_dir}/grain_size_test --spt --counter -Ihpx.stacks.use_guard_pages=0 -min_task_size=${min_task_size} --chunk_size=1 --num_iterations=${ni}  --hpx:threads=${th} --iter_length=${il} --repetitions=1 --hpx:print-counter=/threads/idle-rate  --hpx:print-counter=/threads/time/average --hpx:print-counter=/threads/time/cumulative-overhead --hpx:print-counter=/threads/count/cumulative --hpx:print-counter=/threads/time/average-overhead>>${results_dir}/${node}-sptctr_grain_size_${th}_${min_task_size}_${il}_${ni}.dat
			else

...
then
	...
	tag="${chunk_size}_${block_size_value_row}_${block_size_value_col}_"
else
...
perf stat -e task-clock,cycles,instructions,cache-references,cache-misses ${benchmarks_dir}/${b}_${r}_${node} -only-blaze --hpx:threads=${th} --hpx:bind=balanced --hpx:numa-sensitive &>>${results_dir}/${node}_${backend}-${b}-${th}-${r}-${tag}${mat_size}_perf.dat

export APEX_OTF2=1
${benchmarks_dir}/${b}_${r}_${node} -only-blaze --hpx:threads=${th} --hpx:bind=balanced --hpx:numa-sensitive>>${results_dir}/${node}_${backend}-${b}-${th}-${r}-${tag}${mat_size}.dat

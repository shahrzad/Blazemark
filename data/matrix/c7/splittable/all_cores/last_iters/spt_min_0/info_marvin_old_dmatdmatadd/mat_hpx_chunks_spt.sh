...
for b in ${benchmarks[@]}
	...
	cp ${blazemark_dir}/scripts/mat_hpx_chunks_spt.sh ${results_dir}/info_${node}_${b}/
				...
				for th in "${thr[@]}"
					do 	
						...
						then
							...
							${benchmarks_dir}/${b}_${r}_${node} -only-blaze --hpx:threads=${th} --hpx:bind=balanced --hpx:numa-sensitive --hpx:ini=hpx.thread_queue.min_tasks_to_steal_staged=0>>${results_dir}/${node}_spt-${b}-${th}-${r}-${chunk_size}-${block_size_row}-${block_size_col}-${mat_size}.dat
						fi

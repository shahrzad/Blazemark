...
do
index=91
num=$((end_line-index-1))
for l_num in $(seq $num)
do
cd $blaze_dir
        git checkout $param_filename

	for p in $(seq $((end_line-50)))
	do
		line_number=$((49+p))
		s='\/\/('
    ...
    s='\/\/'
    line_number=$((l_num+index))
    sed -i "${line_number}s/${s}//" $param_filename
mat_size=$(sed -n ${line_number}p "${param_filename}"| cut -d '(' -f2 |cut -d ')' -f1)
mat_size="${mat_size#"${mat_size%%[![:space:]]*}"}"
...
cd ${blazemark_dir}/scripts
	...
	do
		...
		for i in $(seq 1)
		do
echo ${i}-${b}-${th}-${r}-${chunk_size}-${block_size_row}-${block_size_col}-${mat_size}.dat
		    perf stat -e cache-misses ${benchmarks_dir}/${b}_${r} -only-blaze --hpx:threads=${th} --hpx:bind=balanced --hpx:numa-sensitive --hpx:print-counter=/threads/idle-rate  --hpx:print-counter=/threads/time/average --hpx:print-counter=/threads/time/cumulative-overhead --hpx:print-counter=/threads/count/cumulative --hpx:print-counter=/threads/time/average-overhead>>${results_dir}/${i}-${b}-${th}-${r}-${chunk_size}-${block_size_row}-${block_size_col}-${mat_size}.dat
		    echo ${b} "benchmark for" ${r} "finished for "${th} "threads, matrix_size:${mat_size}, chunk size ${c}, block_size row: ${block_size_row} col:${block_size_col} ${i}th time"
		done
		done    
	done
done 

...
for b in ${benchmarks[@]}
	...
	for line_number in $(seq $((start_line+1)) $((start_line+length)))
		...
		block_size_row=$(python -c "from math import ceil;print (2-$mat_size%2)")
                ./change_hpx_parameters.sh BLAZE_HPX_MATRIX_BLOCK_SIZE_ROW "${block_size_row}"

	        for th in "${thr[@]}"
		do
			block_size_col=$(python -c "from math import ceil;print (ceil($mat_size/$th))")
			...
			num_chunks=$((num_chunks_1*num_chunks_2))
	                c=$(python -c "from math import ceil;print (ceil($mat_size/$block_size_row))")

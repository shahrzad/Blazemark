...
for b in ${benchmarks[@]}
	...
	param_filename=${blaze_dir}/blazemark/params/$b.prm
        func_filename=${blaze_dir}/blaze/math/smp/hpx/Functions.h
	...
	for line_number in $(seq $start_line $((end_line-1)))
	do
		s='\/\/('
		sed -i "${line_number}s/${s}/(/" $param_filename
		...
		do
		        cd ${blaze_dir}
		        git checkout $func_filename
		        sed -i "73s/4/$m/" $func_filename	
			modified=$(sed -n 73' p' ${func_filename})
			echo $modified
